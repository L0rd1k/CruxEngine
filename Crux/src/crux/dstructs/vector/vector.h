...
public:
    ...
    explicit vector(size_type size, const_reference val = value_type(), const allocator_type& alloc = allocator_type())
        : VecBase(size, alloc) {
        crux::uninitialized_fill(this->_start, size, val, this->_alloc);  //> copy elements
    ...
    vector(const vector& vec)
        ...
        this->_finish = crux::uninitialized_copy(vec.begin(), vec.end(), this->_start, this->_alloc);
    }
    ...
    vector& operator=(const vector& vec) {
        ...
        if (this->capacity() < vec.size()) {  //> allocate new vector representation:
            ...
            try {
                crux::uninitialized_copy(vec.begin(), vec.end(), result, this->_alloc);  //> copy allocation
            } catch (...) {
            ...
            crux::uninitialized_copy(vec._start + size(), vec._finish, this->_finish, this->_alloc);
        }
    ...
    void swap(vector& vec) {
        ...
        crux::_alloc_swap<allocator_type>::do_swap(this->_alloc, vec._alloc);
    }
    ...
    reference at(size_type n) {
        if (n >= this->size()) {
            std::__throw_out_of_range(__N("crux::vector: Pos out of range."));
        }
        return (*this)[n];
    }
    ...
    void reserve(size_type n) {
        if (n > this->max_size()) {
            std::__throw_length_error(__N("crux::vector: Reserve"));
        }
        if (n > this->capacity()) {
    ...
    void shrink_to_fit() {
        crux::_shrink_to_fit<vector>::do_shrink(*this);
    }
...
private:
    ...
    void initialize_range(ForwardIterator first, ForwardIterator last, std::forward_iterator_tag) {
        const size_type n = crux::distance(first, last);
        this->_start = this->get_allocator().allocate(n);
        this->_storage_end = this->_start + n;
        this->_finish = crux::uninitialized_copy(first, last, this->_start, this->_alloc);
    }
        ...
        if (this->_finish != this->_storage_end) {
            ...
            *pos = Type(std::forward<Args>(args)...);
            ...
            if (this->max_size() - this->size() < size_type(1)) {
                std::__throw_length_error(__N("crux::vector insert_aux"));
            }
            ...
            try {
                ...
                n_finish = 0;
                n_finish = crux::uninitialized_move(this->_start, pos.base(), n_start, this->_alloc);
                ++n_finish;
                n_finish = crux::uninitialized_move(pos.base(), this->_finish, n_finish, this->_alloc);
            } catch (...) {
                if (!n_finish) {
    ...
    void fill_assign(size_type size, const_reference val) {
        if (size > this->capacity()) {
            ...
            crux::uninitialized_fill(this->_finish, size - this->size(), val, this->_alloc);
    ...
    void default_append(size_type n) {
        if (n != 0) {
            if (size_type(this->_storage_end - this->_finish) >= n) {
                crux::uninitialized_default(this->_finish, n, this->_alloc);
                this->_finish += n;
            } else {
                size_type lens = 0;
                if (this->max_size() - this->size() < n) {
                    std::__throw_length_error(__N("crux::vector default_append"));
                }
                ...
                pointer n_finish(n_start);
                try {
                    n_finish = crux::uninitialized_move(this->_start, this->_finish, n_start, this->_alloc);
                    crux::uninitialized_default(n_finish, n, this->_alloc);
                    n_finish += n;
                } catch (...) {
                    std::_Destroy(n_start, n_finish, this->_alloc);
                    this->get_allocator().deallocate(n_start, lens);
    ...
    pointer allocate_and_copy(size_type n, ForwardIterator first, ForwardIterator last) {
        ...
        try {
            crux::uninitialized_copy(first, last, result, this->_alloc);
            return result;
        } catch (...) {
...
inline void swap(crux::vector<T, Alloc>& x, crux::vector<T, Alloc>& y) {
    x.swap(y);
}
